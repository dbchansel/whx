...
namespace DTcms.Web.UI
{
    public partial class BasePage : System.Web.UI.Page
    {
        ...
        protected DataTable get_article_list(string channel_name, int category_id, int page_size, int page_index, string strwhere, out int totalcount, out string pagelist, string _key, params object[] _params)
        {
            ...
            if (!string.IsNullOrEmpty(channel_name))
            {
                dt = new BLL.article().ArticleList(channel_name, category_id, page_size, page_index, strwhere, "info.sort_id asc,info.add_time desc", out totalcount).Tables[0];
        ...
        /// <param name="_params">传输参数</param>
        /// <returns>DataTable</returns>
        protected DataTable get_article_list_record(string channel_name, int category_id, int page_size, int page_index, string strwhere, out int totalcount, out string pagelist, string _key, params object[] _params)
        {
            DataTable dt = new DataTable();
            if (!string.IsNullOrEmpty(channel_name))
            {
        ...
        /// <returns>DataTable</returns>
        protected DataTable get_VoteInfo(string id)
        {
            DataTable dt = new DataTable();
            if (!string.IsNullOrEmpty(id))
            {
                Model.Vote obj = new BLL.Vote().GetModel(int.Parse(id));
