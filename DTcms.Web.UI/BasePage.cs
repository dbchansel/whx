...
namespace DTcms.Web.UI
{
    public partial class BasePage : System.Web.UI.Page
    {
        ...
        public string linkurl(string _key, params object[] _params)
        {
            ...
            foreach (Model.url_rewrite_item item in model.url_rewrite_items)
            {
                ...
                if (IsUrlMatch(item, _params))
                {
                    ...
                    if (config.staticstatus > 0)
                    {
                        string ss = linkStartString + string.Format(GetUrlExtension(item.path, config.staticextension), _params) + (_key == "goods_show" ? "?record=1" : "");
                        return linkStartString + string.Format(GetUrlExtension(item.path, config.staticextension), _params) + (_key == "goods_show" ? "?record=1":"");
                    }
                    else
                    {
        ...
        public string linkurl_record(string _key, params object[] _params)
        {
            Hashtable ht = new BLL.url_rewrite().GetList(); //获得URL配置列表
            Model.url_rewrite model = ht[_key] as Model.url_rewrite; //查找指定的URL配置节点
