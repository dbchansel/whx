...
namespace DTcms.DAL
{
    ...
    public partial class VoteAuto
    {
        ...
        public VoteAuto(string _databaseprefix)
        {
        ...
        public bool Exists(int id)
        {
            ...
            strSql.Append("select count(1) from  " + databaseprefix + "VoteAutoRules");
        ...
        /// 增加一条数据
        /// </summary>
        public int Add(Model.VoteAuto model)
        {
            StringBuilder strSql = new StringBuilder();
            ...
            strSql.Append("insert into  " + databaseprefix + "VoteAutoRules(");
        ...
        /// 更新一条数据
        /// </summary>
        public bool Update(Model.VoteAuto model)
        {
            StringBuilder strSql = new StringBuilder();
            ...
            PropertyInfo[] pros = model.GetType().GetProperties();
            List<SqlParameter> paras = new List<SqlParameter>();
            strSql.Append("update  " + databaseprefix + "VoteAutoRules set ");
            foreach (PropertyInfo pi in pros)
            {
                //如果不是主键则追加sql字符串
                if (!pi.Name.Equals("id"))
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from " + databaseprefix + "VoteAutoRules ");
            strSql.Append(" where id=@id");
            SqlParameter[] parameters = {
                    new SqlParameter("@id", SqlDbType.Int,4)};
            parameters[0].Value = id;
        ...
        /// 得到一个对象实体
        /// </summary>
        public Model.VoteAuto GetModel(int id)
        {
            StringBuilder strSql = new StringBuilder();
            StringBuilder str1 = new StringBuilder();
            ...
            strSql.Append(" from " + databaseprefix + "VoteAutoRules");
            strSql.Append(" where id=@id");
            SqlParameter[] parameters = {
                    new SqlParameter("@id", SqlDbType.Int,4)};
            parameters[0].Value = id;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * FROM " + databaseprefix + "VoteAutoRules");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
        ...
        /// 得到一个对象实体
        /// </summary>
        public Model.VoteAuto GetModel(string nav_name)
        {
            StringBuilder strSql = new StringBuilder();
            StringBuilder str1 = new StringBuilder();
            Model.VoteAuto model = new Model.VoteAuto();
        ...
        /// 得到一个对象实体，带事务
        /// </summary>
        public Model.VoteAuto GetModel(SqlConnection conn, SqlTransaction trans, string nav_name)
        {
            StringBuilder strSql = new StringBuilder();
            StringBuilder str1 = new StringBuilder();
            Model.VoteAuto model = new Model.VoteAuto();
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top 1 id from " + databaseprefix + "VoteAutoRules");
            strSql.Append(" where name=@nav_name");
            SqlParameter[] parameters = {
                    new SqlParameter("@nav_name", SqlDbType.NVarChar,50)};
            parameters[0].Value = nav_name;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select id,parent_id from " + databaseprefix + "VoteAutoRules");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update " + databaseprefix + "VoteAutoRules set " + strValue);
            strSql.Append(" where id=" + id);
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update " + databaseprefix + "VoteAutoRules set name=@new_name");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * FROM " + databaseprefix + "VoteAutoRules where VoteId=" + id + " " );
        ...
        /// 将对象转换实体
        /// </summary>
        public Model.VoteAuto DataRowToModel(DataRow row)
        {
            Model.VoteAuto model = new Model.VoteAuto();
            if (row != null)
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * FROM dt_VoteAutoRules");
            strSql.Append(" order by sort_id asc,id asc");
            DataSet ds = DbHelperSQL.Query(strSql.ToString());
            //重组列表
            DataTable oldData = ds.Tables[0] as DataTable;
        ...
        public int GetCurDayCurVoteCount(string openid, int id)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * FROM dt_VoteAutoRules");
            strSql.Append(" where VoteAutoId='" + openid + "' and PointId=" + id + " and year(CreateDate)=" + DateTime.Now.Year + " and MONTH(CreateDate)=" + DateTime.Now.Month + " and DAY(CreateDate)=" + DateTime.Now.Day);
            DataSet ds = DbHelperSQL.Query(strSql.ToString());
            //重组列表
            DataTable oldData = ds.Tables[0] as DataTable;
        ...
        public int GetCurDayCount(string openid)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * FROM dt_VoteAutoRules");
            strSql.Append(" where VoteAutoId='" + openid + "' and year(CreateDate)=" + DateTime.Now.Year + " and MONTH(CreateDate)=" + DateTime.Now.Month + " and DAY(CreateDate)=" + DateTime.Now.Day);
            DataSet ds = DbHelperSQL.Query(strSql.ToString());
            //重组列表
            DataTable oldData = ds.Tables[0] as DataTable;
            return oldData.Rows.Count;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top 1 CreateDate from " + databaseprefix + "VoteAutoRules where VoteAutoId='" + openid + "'");
