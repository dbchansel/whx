...
namespace DTcms.DAL
{
    ...
    public partial class users
    {
        ...
        {
            //尝试用户名取得Salt 从manager表中
            StringBuilder strSqlManager = new StringBuilder();
            strSqlManager.Append("select top 1 salt from " + databaseprefix + "manager");
            strSqlManager.Append(" where user_name=@user_name");
            SqlParameter[] parametersManager = {
                    new SqlParameter("@user_name", SqlDbType.NVarChar,100)};
            parametersManager[0].Value = user_name;
            string saltManager = Convert.ToString(DbHelperSQL.GetSingle(strSqlManager.ToString(), parametersManager));
            if (!string.IsNullOrEmpty(saltManager))
            {
                return saltManager;
            }
