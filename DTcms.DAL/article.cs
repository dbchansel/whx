...
namespace DTcms.DAL
{
	...
	public partial class article
	{
        ...
        /// 根据频道名称获得查询分页数据
        /// </summary>
        public DataSet ArticleList_record(string channel_name, int category_id, int pageSize, int pageIndex, string strWhere, string filedOrder, out int recordCount)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select info.*,recordinfo.countnum FROM " + databaseprefix + DTKeys.TABLE_CHANNEL_ARTICLE + channel_name +" info ");
            strSql.Append(" LEFT JOIN  (select parent_id,COUNT(id) countnum FROM dt_erp_record_Info GROUP BY parent_id) recordinfo on recordinfo.parent_id=info.id ");
            strSql.Append(" where datediff(d,info.add_time,getdate())>=0");
            if (category_id > 0)
            {
                strSql.Append(" and info.category_id in(select id from " + databaseprefix + "article_category where class_list like '%," + category_id + ",%')");
            }
            if (strWhere.Trim() != "")
            {
                strSql.Append(" and " + strWhere);
