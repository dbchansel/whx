...
namespace DTcms.DAL
{
    ...
    public partial class VoteJoin
    {
        ...
        public VoteJoin(string _databaseprefix)
        {
        ...
        public bool Exists(int id)
        {
            ...
            strSql.Append("select count(1) from  " + databaseprefix + "VoteJoin_Info");
        ...
        public int Add(Model.VoteJoin model)
        {
            ...
            strSql.Append("insert into  " + databaseprefix + "VoteJoin_Info(");
        ...
        public bool Update(Model.VoteJoin model)
        {
            ...
            PropertyInfo[] pros = model.GetType().GetProperties();
            List<SqlParameter> paras = new List<SqlParameter>();
            strSql.Append("update  " + databaseprefix + "VoteJoin_Info set ");
            foreach (PropertyInfo pi in pros)
            {
                //如果不是主键则追加sql字符串
                if (!pi.Name.Equals("id"))
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from " + databaseprefix + "VoteJoin_Info ");
            strSql.Append(" where id=@id");
            SqlParameter[] parameters = {
                    new SqlParameter("@id", SqlDbType.Int,4)};
            parameters[0].Value = id;
        ...
        public int GetJoinNum(int id)
        {
            ...
            strSql.Append(" JoinNumber ");
            strSql.Append(" FROM  " + databaseprefix + "VoteJoin_Info ");

            strSql.Append(" where VoteId=" + id);
            
            strSql.Append(" order by JoinNumber desc");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * FROM " + databaseprefix + "VoteJoin_Info");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
        ...
        /// 得到一个对象实体
        /// </summary>
        public Model.VoteJoin GetModel(string nav_name)
        {
            StringBuilder strSql = new StringBuilder();
            StringBuilder str1 = new StringBuilder();
            Model.VoteJoin model = new Model.VoteJoin();
        ...
        /// 得到一个对象实体，带事务
        /// </summary>
        public Model.VoteJoin GetModel(SqlConnection conn, SqlTransaction trans, string nav_name)
        {
            StringBuilder strSql = new StringBuilder();
            StringBuilder str1 = new StringBuilder();
            Model.VoteJoin model = new Model.VoteJoin();
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top 1 id from " + databaseprefix + "VoteJoin_Info");
            strSql.Append(" where name=@nav_name");
            SqlParameter[] parameters = {
                    new SqlParameter("@nav_name", SqlDbType.NVarChar,50)};
            parameters[0].Value = nav_name;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select id,parent_id from " + databaseprefix + "VoteJoin_Info");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update " + databaseprefix + "VoteJoin_Info set " + strValue);
            strSql.Append(" where id=" + id);
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update " + databaseprefix + "VoteJoin_Info set name=@new_name");
        ...
        /// 将对象转换实体
        /// </summary>
        public Model.VoteJoin DataRowToModel(DataRow row)
        {
            Model.VoteJoin model = new Model.VoteJoin();
            if (row != null)
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append(" select * FROM dt_VoteJoin_Info ");
            strSql.Append(" where JoinSta='1' and VoteId=" + id);
            DataSet ds = DbHelperSQL.Query(strSql.ToString());
            //重组列表
            DataTable oldData = ds.Tables[0] as DataTable;
        ...
        public DataTable GetList2(int id)
        {
            ...
            strSql.Append("select * FROM dt_VoteJoin_Info");
            strSql.Append(" where JoinSta='1' and  VoteId=" + id+" ");
            strSql.Append(" order by VoteNum desc ");
            DataSet ds = DbHelperSQL.Query(strSql.ToString());
            //重组列表
            DataTable oldData = ds.Tables[0] as DataTable;
            oldData.Columns.Add("Rank", typeof(int));
