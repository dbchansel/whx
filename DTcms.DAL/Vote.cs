...
namespace DTcms.DAL
{
    ...
    public partial class Vote
    {
        ...
        public Vote(string _databaseprefix)
        {
        ...
        public bool Exists(int id)
        {
            ...
            strSql.Append("select count(1) from  " + databaseprefix + "Vote_Info");
        ...
        /// 增加一条数据
        /// </summary>
        public int Add(Model.Vote model)
        {
            StringBuilder strSql = new StringBuilder();
            ...
            strSql.Append("insert into  " + databaseprefix + "Vote_Info(");
        ...
        /// 更新一条数据
        /// </summary>
        public bool Update(Model.Vote model)
        {
            StringBuilder strSql = new StringBuilder();
            ...
            PropertyInfo[] pros = model.GetType().GetProperties();
            List<SqlParameter> paras = new List<SqlParameter>();
            strSql.Append("update  " + databaseprefix + "Vote_Info set ");
            foreach (PropertyInfo pi in pros)
            {
                //如果不是主键则追加sql字符串
                if (!pi.Name.Equals("id"))
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from " + databaseprefix + "Vote_Info ");
            strSql.Append(" where id=@id");
            SqlParameter[] parameters = {
                    new SqlParameter("@id", SqlDbType.Int,4)};
            parameters[0].Value = id;
        ...
        /// 得到一个对象实体
        /// </summary>
        public Model.Vote GetModel(int id)
        {
            StringBuilder strSql = new StringBuilder();
            StringBuilder str1 = new StringBuilder();
            ...
            strSql.Append(" from " + databaseprefix + "Vote_Info");
            strSql.Append(" where id=@id");
            SqlParameter[] parameters = {
                    new SqlParameter("@id", SqlDbType.Int,4)};
            parameters[0].Value = id;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * FROM " + databaseprefix + "Vote_Info");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
        ...
        /// 得到一个对象实体
        /// </summary>
        public Model.Vote GetModel(string nav_name)
        {
            StringBuilder strSql = new StringBuilder();
            StringBuilder str1 = new StringBuilder();
            Model.Vote model = new Model.Vote();
        ...
        /// 得到一个对象实体，带事务
        /// </summary>
        public Model.Vote GetModel(SqlConnection conn, SqlTransaction trans, string nav_name)
        {
            StringBuilder strSql = new StringBuilder();
            StringBuilder str1 = new StringBuilder();
            Model.Vote model = new Model.Vote();
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top 1 id from " + databaseprefix + "Vote_Info");
            strSql.Append(" where name=@nav_name");
            SqlParameter[] parameters = {
                    new SqlParameter("@nav_name", SqlDbType.NVarChar,50)};
            parameters[0].Value = nav_name;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select id,parent_id from " + databaseprefix + "Vote_Info");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update " + databaseprefix + "Vote_Info set " + strValue);
            strSql.Append(" where id=" + id);
            return DbHelperSQL.ExecuteSql(strSql.ToString()) > 0;
        }
        public bool isAllowNum(int id)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * from " + databaseprefix + "Vote_Info ");
            strSql.Append(" where id=" + id + " and Rules_MinMember >= JoinMemberNum");
            return (DbHelperSQL.ExecuteSql(strSql.ToString()) > 0);
        }
        /// <summary>
        /// 修改一列数据
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update " + databaseprefix + "Vote_Info set name=@new_name");
        ...
        /// 将对象转换实体
        /// </summary>
        public Model.Vote DataRowToModel(DataRow row)
        {
            Model.Vote model = new Model.Vote();
            if (row != null)
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * FROM dt_Vote_Info");
            strSql.Append(" order by sort_id asc,id asc");
            DataSet ds = DbHelperSQL.Query(strSql.ToString());
            //重组列表
            DataTable oldData = ds.Tables[0] as DataTable;
