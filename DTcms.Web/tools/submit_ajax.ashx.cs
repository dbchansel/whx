...
namespace DTcms.Web.tools
{
    ...
    public class submit_ajax : IHttpHandler, IRequiresSessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            switch (action)
            {
                case "Vote": //投票
                    Vote_Fun(context);
                    break;
                case "save_goods_record": //保存record
                    save_goods_record(context);
                break;
                ...
                case "Vote_V": //虚假投票
                    ...
                    break;
                case "Browse_V": //虚假浏览
                    Browse_V_Fun(context);
                    break;
                case "VoteJoin": //申请加入
                    VoteJoin(context);
                break;
                case "GiftVote": //真实打赏
                    GiftVote_Fun(context);
                    break;
                case "GiftVote_V": //虚拟打赏
                    GiftVote_V_Fun(context);
                    break;
                ...
                case "view_cart_count": //输出当前购物车总数
                    ...
                    break;
                case "order_save_vote": //保存礼物订单
                    order_save_vote(context);
                    break;
            }
        }

        #region 提交评论的处理方法OK===========================
        private void VoteJoin(HttpContext context)
        {
            string ss = context.Request.Form["JoinImg1"].ToString();
            string JoinContent = context.Request.Form["JoinContent"].ToString();
            string JoinAddress = context.Request.Form["JoinAddress"].ToString();
            string JoinTel = context.Request.Form["JoinTel"].ToString();
            string JoinName = context.Request.Form["JoinName"].ToString();
            string openid = context.Request.Form["openid"].ToString();
            DateTime dt = DateTime.Now;
            string imglim = openid + "" + dt.Year + "" + dt.Month + "" + dt.Day + "" + dt.Hour + "" + dt.Minute + "" + dt.Second;
            string JoinImg1 = Base64StrToImage(context.Request.Form["JoinImg1"].ToString(), "../upload/img/"  + "JoinImg1_" + imglim + ".png");
            string JoinImg2 = Base64StrToImage(context.Request.Form["JoinImg2"].ToString(), "../upload/img/" + "JoinImg2_" + imglim + ".png");
            string JoinImg3 = Base64StrToImage(context.Request.Form["JoinImg3"].ToString(), "../upload/img/" + "JoinImg3_" + imglim + ".png");
            string JoinImg4 = Base64StrToImage(context.Request.Form["JoinImg4"].ToString(), "../upload/img/" + "JoinImg4_" + imglim + ".png");
            string JoinImg5 = Base64StrToImage(context.Request.Form["JoinImg5"].ToString(), "../upload/img/" + "JoinImg5_" + imglim + ".png");
            string JoinImg6 = Base64StrToImage(context.Request.Form["JoinImg6"].ToString(), "../upload/img/" + "JoinImg6_" + imglim + ".png");
            string JoinImg7 = Base64StrToImage(context.Request.Form["JoinImg7"].ToString(), "../upload/img/" + "JoinImg7_" + imglim + ".png");
            string JoinImg8 = Base64StrToImage(context.Request.Form["JoinImg8"].ToString(), "../upload/img/" + "JoinImg8_" + imglim + ".png");
            string JoinImg9 = Base64StrToImage(context.Request.Form["JoinImg9"].ToString(), "../upload/img/" + "JoinImg9_" + imglim + ".png");
            string JoinImg10 = Base64StrToImage(context.Request.Form["JoinImg10"].ToString(), "../upload/img/" + "JoinImg10_" + imglim + ".png");

            int VoteId = int.Parse(context.Request.Form["id"].ToString());
            int JoinNumber = new BLL.VoteJoin().GetJoinNum(VoteId);
            Model.VoteJoin vj = new Model.VoteJoin();
            vj.JoinContent = JoinContent;

            vj.JoinAddress = JoinAddress;
            vj.JoinTel = JoinTel;
            vj.JoinName = JoinName;
            vj.JoinNumber = JoinNumber.ToString();
            vj.JoinImg1 = JoinImg1;
            vj.JoinImg2 = JoinImg2;
            vj.JoinImg3 = JoinImg3;
            vj.JoinImg4 = JoinImg4;
            vj.JoinImg5 = JoinImg5;
            vj.JoinImg6 = JoinImg6;
            vj.JoinImg7 = JoinImg7;
            vj.JoinImg8 = JoinImg8;
            vj.JoinImg9 = JoinImg9;
            vj.JoinImg10 = JoinImg10;
            vj.VoteId = VoteId;
            vj.JoinSta = 0;
            vj.VoteNum = 0;
            vj.GiftNum = "0";
            if (new BLL.VoteJoin().Add(vj) > 0)
            {
                BLL.Vote bllv = new BLL.Vote();
                Model.Vote vm = bllv.GetModel(vj.VoteId);
                if (vm != null)
                {
                    vm.JoinMemberNum++;
                    bllv.Update(vm);
                }
                context.Response.Write("{\"status\": 1, \"msg\": \"加入成功！\"}");
                return;
            }
            else
            {
                context.Response.Write("{\"status\": 0, \"msg\": \"加入失败！\"}");
                return;
            }
        }
        ...
        public static string Base64StrToImage(string base64Str, string savePath)
        {
            ...
            var ret = savePath.Substring(3,(savePath.Length - 3));
            ...
            try
            {
                ...
                if (bitmap != null)
                {
                    ...
                    bmpNew.Save(GetPath(savePath), suffixName);
                    bmpNew.Dispose();
        ...
        /// 获取文件相对路径映射的物理路径
        /// </summary>
        /// <param name="virtualPath">文件的相对路径，如：Images/logo.png</param>
        public static string GetPath(string virtualPath)
        {
            return HttpContext.Current != null ?
             HttpContext.Current.Server.MapPath(virtualPath) ://Web方式
             Path.Combine(AppDomain.CurrentDomain.BaseDirectory, virtualPath.Replace("/", "\\"));//非Web方式
        }
        /// <summary>
        /// 创建目录
        /// </summary>
        /// <param name="dir">要创建的目录路径包括目录名（相对或物理路径）</param>
        /// <param name="isPhyPath">是否为物理路径</param>
        public static void CreateDir(string dir, bool isPhyPath = false)
        {
            if (dir.Length == 0) return;

            dir = isPhyPath ? dir : GetPath(dir);
        ...
        private void save_goods_record(HttpContext context)
        {
            int id = DTRequest.GetQueryInt("id");
            string prices = DTRequest.GetQueryString("prices");
            string trannsform_type = DTRequest.GetQueryString("trannsform_type");
            string remarks = DTRequest.GetQueryString("remarks");
            string transform_num = DTRequest.GetQueryString("transform_num");

            Model.erp_record_Info _records = new Model.erp_record_Info();
            _records.parent_id = id.ToString();
            _records.remarks = remarks;
            _records.trannsform_type = int.Parse(trannsform_type.ToString());
            _records.transform_num = transform_num;
            if (trannsform_type == "1")
            {
                _records.real_prices = prices;
            }
            else
            {
                _records.sale_prices = prices;
            }
            ...
            if (new BLL.erp_record_Info().Add(_records) > 0)
            {
                if (!string.IsNullOrEmpty(id.ToString()))
                {
                    ...
                    bool s = new BLL.article_goods().upsertNum(curNum.ToString(), id.ToString());

                    context.Response.Write("{\"status\": 1, \"msg\": \"完成！\"}");
                    return;
                }
            }
        ...
        private void Vote_Fun(HttpContext context)
        {
            int id = DTRequest.GetQueryInt("id");
            string openid = DTRequest.GetQueryString("openid");
            int VoteType = 0;
            BLL.VoteJoin bll = new BLL.VoteJoin();
            Model.VoteJoin vj = bll.GetModel(id);
            //此处为判断  
            //if(判断)
            Model.Vote modelV = new BLL.Vote().GetModel(vj.VoteId);
            if (modelV.Activity_VoteEDTime <= DateTime.Now)
            {
                context.Response.Write("{\"status\": 0, \"msg\": \"当前活动已结束！\"}");
                return;
            }

            #region  判断是否允许投票
            //1，判断是否个人超过每日每用户投票数量 Rules_PDUVotes
            int Rules_PDUVotes = modelV.Rules_PDUVotes;
            int curRules_PDUVotes = new BLL.VoteRecord().GetCurDayCurVoteCount(openid, id);
            if (curRules_PDUVotes >= Rules_PDUVotes)
            {
                context.Response.Write("{\"status\": 2, \"msg\": \"您当前助力次数已达上线，请明天再次助力该好友吧！\"}");
                return;
            }
            //2，判断是否个人超过每日每人投票数量 Rules_DPVotes
            int Rules_DPVotes = modelV.Rules_DPVotes;
            //3，判断是否个人超过每人最多投票数量 Rules_MaxVotes
            int Rules_MaxVotes = modelV.Rules_MaxVotes == 0 ?  99999: modelV.Rules_MaxVotes;
            int curRules_MaxVotes = new BLL.VoteRecord().GetCurDayCount(openid);
            if (curRules_MaxVotes >= Rules_MaxVotes)
            {
                context.Response.Write("{\"status\": 3, \"msg\": \"您已超过当日最多投票数，明天再来助力该好友吧！\"}");
                return;
            }
            
            //3，判断是否个人超过投票时间间隔分钟 Rules_DateInterval
            int Rules_DateInterval = modelV.Rules_DateInterval;
            int curRules_DateInterval = new BLL.VoteRecord().GetDateInterval(openid);
            if (curRules_DateInterval < Rules_DateInterval)
            {
                context.Response.Write("{\"status\": 4, \"msg\": \"助力时间间隔太短，请稍后再试！\"}");
                return;
            }
            #endregion
            //更新数据
            if (vj != null)
            {
                ...
                {
                    ////真实投票
                    //Model.Vote v = new BLL.Vote().GetModel(vj.VoteId);
                    //v.Gift_Votes1_ZS++;
                    //new BLL.Vote().Update(v);
                    ...
                    vr.VoteRecordId = openid;
                    vr.Other = "常规用户投票";
                    new BLL.VoteRecord().Add(vr);
                    context.Response.Write("{\"status\": 1, \"msg\": \"为Ta助力成功！\"}");
                    return;
                }
                else {
                    context.Response.Write("{\"status\": 0, \"msg\": \"对不起，投票失败！\"}");
                    return;
                }
            }
            else
        ...
        //Vote_V_Fun
        private void Vote_V_Fun(HttpContext context)
        {
            int id = DTRequest.GetQueryInt("id");
            int VoteNum = DTRequest.GetQueryInt("VoteNum");

            BLL.VoteJoin bll = new BLL.VoteJoin();
            Model.VoteJoin model = bll.GetModel(id);

            model.VoteNum += VoteNum;
            model.VoteNum_XN += VoteNum;
            model.VoteNum = (model.VoteNum >= 0 ? model.VoteNum : 0);
            if (bll.Update(model))
            {
                BLL.VoteRecord bllRecord = new BLL.VoteRecord();
                Model.VoteRecord vr = new Model.VoteRecord();
                vr.PointId = id;
                vr.SetVoteNum = VoteNum;
                vr.VoteType = 2;
                vr.VoteRecordId = Utils.GetCookie("AdminUserId");
                vr.VotePlatform = "管理后台管理员增加";
                if (bllRecord.Add(vr) > 0)
                {
                    context.Response.Write("{\"status\": 1, \"msg\": \"成功！\"}");
                }
            }
        }
        private void GiftVote_Fun(HttpContext context)
        {
            int id = DTRequest.GetQueryInt("id");
            int VoteNum = DTRequest.GetQueryInt("VoteNum");
            string GiftNum = DTRequest.GetFormString("GiftNum");
            context.Response.Write("{\"status\": 1, \"msg\": \"为Ta助力成功！\"}");
        }
        private void GiftVote_V_Fun(HttpContext context)
        {
            int id = DTRequest.GetQueryInt("id");
            int GiftNum = DTRequest.GetQueryInt("GiftNum");

            BLL.VoteJoin bll = new BLL.VoteJoin();
            Model.VoteJoin model = bll.GetModel(id);

            model.VoteNum += GiftNum * 3;
            model.GiftNum = (int.Parse(model.GiftNum) +  GiftNum * 3).ToString();
            model.GiftNum_XN += GiftNum * 3;
            ...
            if (bll.Update(model))
            {
                ...
                Model.VoteRecord vr = new Model.VoteRecord();
                vr.PointId = id;
                vr.SetVoteNum = GiftNum * 3;
                vr.VoteType = 4;
                vr.VoteRecordId = Utils.GetCookie("AdminUserId");
                vr.VotePlatform = "管理员增加礼物";
                if (bllRecord.Add(vr) > 0)
                {
                    context.Response.Write("{\"status\": 1, \"msg\": \"成功！\"}");
                }
        ...
        private void Browse_V_Fun(HttpContext context)
        {
            int id = DTRequest.GetQueryInt("id");
            int BrowseNum = DTRequest.GetQueryInt("BrowseNum");

            BLL.VoteJoin bll = new BLL.VoteJoin();
            Model.VoteJoin model = bll.GetModel(id);

            model.BrowseNum += BrowseNum;
            model.BrowseNum_XN += BrowseNum;

            model.BrowseNum = (model.BrowseNum >= 0 ? model.BrowseNum : 0);
            if (bll.Update(model))
            {
                context.Response.Write("{\"status\": 1, \"msg\": \"成功！\"}");
            }
        ...
        private void order_save(HttpContext context)
        {
            ...
            int gift = DTRequest.GetFormInt("gift");
        ...
        private void order_save_vote(HttpContext context)
        {
            //获取传参信息===================================
            int site_id = DTRequest.GetInt("site_id", 0); //站点ID
            int gift = DTRequest.GetQueryInt("gift");
            string openid = DTRequest.GetQueryString("openid");
            string sitepath = new BLL.sites().GetBuildPath(site_id); //站点目录
            int voteid = DTRequest.GetInt("voteid", 0); //voteid

            Model.VoteJoin modelvj = new BLL.VoteJoin().GetModel(voteid);
            Model.Vote modelv = new BLL.Vote().GetModel(modelvj.VoteId);
            if (modelv.Activity_VoteEDTime <= DateTime.Now)
            {
                context.Response.Write("{\"status\":0, \"msg\":\"活动已结束！\"}");
                return;
            }
            ...
            //检查支付方式
            Model.payment payModel = new BLL.payment().GetModel(8);
            if (payModel == null)
            {
                context.Response.Write("{\"status\":0, \"msg\":\"对不起，支付方式不存在或已删除！\"}");
                return;
            ...
            model.payment_id = 5;       
            model.express_status = 1;
            model.payable_amount = decimal.Parse(gift.ToString());
            model.order_amount = decimal.Parse(gift.ToString());
            model.voteid = voteid;
            /*
            model.payable_amount = gift;
        
            //订单总金额=实付商品金额+运费+支付手续费+税金
            model.order_amount = gift;
            */
            model.add_time = DateTime.Now;
            int result = new BLL.orders().Add(model);
            if (result < 1)
            ...
            //提交成功，返回URL
            context.Response.Write("{\"status\":1, \"url\":\""
                + new Web.UI.BasePage().getlink(sitepath, new Web.UI.BasePage().linkurl("payment", "?action=confirmVote&order_no=" + model.order_no)) + "\", \"msg\":\"恭喜您，订单已成功提交！\", \"order_no\":\""+ model.order_no + "\", \"order_amount\":\"" + model.order_amount + "\"}");
            return;
        }
        #endregion
