    ...
    public partial class jsapipay : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                data.SetValue("body", GetUtf8(subject1)); //商品描述
                ...
                int voteNum = Decimal.ToInt32(model.payable_amount) * 3;

                errstr += "_voteNum";
                int curvoteid = model.voteid;
                Model.VoteJoin vj = new BLL.VoteJoin().GetModel(curvoteid);
                if (vj != null)
                {
                    vj.VoteNum += voteNum;
                    vj.GiftNum_ZS += Decimal.ToInt32(model.payable_amount);


                    //充值
                    vj.GiftNum = (Convert.ToDecimal(vj.GiftNum) + model.payable_amount).ToString();
                    if (new BLL.VoteJoin().Update(vj))
                    {
                        Model.Vote v = new BLL.Vote().GetModel(vj.VoteId);
                        v.GiftSum += Decimal.ToInt32(model.payable_amount);
                        v.Gift_Votes1_ZS += Decimal.ToInt32(model.payable_amount);
                        new BLL.Vote().Update(v);

                        //支付成功后跳转的URL
                        returnUrl = "../../../aspx/mobile/payment_vote.aspx?action=succeed&order_no=" + order_no;
                        errstr += "_支付成功后跳转的URL";
                    }
            ...
            {
                returnUrl = "../../../aspx/mobile/payment_vote.aspx?action=error&order_no=" + order_no;
            }

        }
        private string GetUtf8(string unicodeString)
        {
