...
namespace DTcms.Web.api.oauth
{
    public partial class index : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            int VoteId = DTRequest.GetQueryInt("vote"); //VoteId
            type = DTRequest.GetQueryInt("type"); //1 投票
            int votememberid = DTRequest.GetQueryInt("votememberid"); //1 投票
            if (site_oauth_id == 0)
            {
            ...
            Session["votememberid"] = votememberid;
            ...
            switch (config.oauth_name)
            {
                ...
                case "sina":
                    ...
                    break;
                case "weixin"://http://wtp.danni.fun/aspx/mobile/Vote.aspx?id=" + VoteId
                    if (type == 1)
                        send_url = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=" + config.oauth_app_id + "&redirect_uri=" + Utils.UrlEncode(config.return_uri + "?id=" + votememberid+"") + "&response_type=code&scope=snsapi_userinfo&state=" + state + "#wechat_redirect";
                    else
                        send_url = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=" + config.oauth_app_id + "&redirect_uri=" + Utils.UrlEncode(config.return_uri + "?VoteId=" + VoteId + "") + "&response_type=code&scope=snsapi_userinfo&state=" + state + "#wechat_redirect";
                    break;
                case "renren":
