    ...
    public partial class manager_edit : Web.UI.ManagePage
    {
        ...
        private bool DoAdd()
        {
            ...
            if (bll.Add(model) > 0)
            {
                ...
                modelUser.site_id = 1;
                modelUser.group_id = 1;
                modelUser.user_name = model.user_name;
                modelUser.salt = model.salt; //获得6位的salt加密字符串
                modelUser.password = model.password;
                modelUser.email = model.email;
                modelUser.mobile = model.telephone;
                modelUser.reg_ip = "127.0.0.1";
                modelUser.reg_time = DateTime.Now;
                //设置用户状态
                modelUser.status = 0; //正常
                int count =  new BLL.users().Add(modelUser);
                string ss = "";
