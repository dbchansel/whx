...
namespace DTcms.Web.admin.article
{
    public partial class article_list : Web.UI.ManagePage
    {
        ...
        protected void btnshow_record(object sender, EventArgs e)
        {
            int count = 0;
            for (int i = 0; i < rptList1.Items.Count; i++)
            {
                ...
                {
                    count++;
                    //this.Page.ClientScript.RegisterStartupScript(this.Page.GetType(), "", "<script>show_goods_list('" + id + "');</script>", true);
                    ClientScript.RegisterClientScriptBlock(this.GetType(), "show_goods_list", "show_goods_list('" + id + "')", true);
                    ExcuteJS("show_goods_list", id);
                    return;
                }
            }
